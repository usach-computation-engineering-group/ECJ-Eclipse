...
public class GEPSubpopulation extends Subpopulation {
	...
	public void populate(EvolutionState state, int thread) {
		...
		if (numDuplicateRetries >= 1)
			h = new Hashtable(individuals.size() / 2); // seems reasonable
		...
		for (int x = 0; x < individuals.size(); x++) {
			for (int tries = 0; tries <= /* Yes, I see that */ numDuplicateRetries; tries++) {
				...
				for (int zeroFitnessTries = 0; zeroFitnessTries <= numZeroFitnessRetries; zeroFitnessTries++) {
					((SimpleProblemForm) (state.evaluator.p_problem)).evaluate(state, individuals.get(x), 0, 0);
					if (individuals.get(x).fitness.fitness() <= 0.0)
						individuals.set(x, species.newIndividual(state, thread));
					else
						break;
				}
