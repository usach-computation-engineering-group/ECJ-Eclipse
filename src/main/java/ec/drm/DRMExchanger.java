...
public class DRMExchanger extends Exchanger{
    ...
    public Population preBreedingExchangePopulation(final EvolutionState state){
	    ...
	    for( int subpop = 0 ; subpop < state.population.subpops.size() ; subpop++ ){
	    	...
	    	immigrantsSelectionMethod.produce(migrationsize,migrationsize, subpop, new ArrayList<Individual>(Arrays.asList(inds)), state, 0,null);
    ...
    public Population postBreedingExchangePopulation(final EvolutionState state){
    	...
    	while( mailbox.size() > 0){
            ...
            for( int i = 0 ; i < n ; i++ ){
                state.population.subpops.get(s).individuals.set(replace[i], exData.individuals[i]);
                state.population.subpops.get(s).individuals.get(replace[i]).evaluated = false; 
            }
    	}
        return state.population;
    }
