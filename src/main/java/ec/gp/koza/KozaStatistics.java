...
public class KozaStatistics extends Statistics implements SteadyStateStatisticsForm
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7219059150428586580L;
        ...
        if (doFull)
            ...
            for(int x=0;x<state.population.subpops.size();x++)
                ...
                for(int y=0;y<state.population.subpops.get(x).individuals.size();y++)
                    {
                    GPIndividual i = 
                        (GPIndividual)(state.population.subpops.get(x).individuals.get(y));
    ...
    public void postEvaluationStatistics(final EvolutionState state)
        ...
        for(int x=0;x<state.population.subpops.size();x++)
            ...
            if (doFull)
                ...
                long[] numNodes = new long[((GPIndividual)(state.population.subpops.get(x).species.i_prototype)).trees.length];
                long[] numDepth = new long[((GPIndividual)(state.population.subpops.get(x).species.i_prototype)).trees.length];
                ...
                state.output.println("Avg Nodes: " + ((double)totNodesPerGen)/state.population.subpops.get(x).individuals.size(), statisticslog);
                state.output.print("Nodes/tree: [", statisticslog);
                for(int tr=0;tr<numNodes.length;tr++)
                    {
                    if (tr>0) state.output.print("|", statisticslog);
                    state.output.print(""+((double)numNodes[tr])/state.population.subpops.get(x).individuals.size(), statisticslog);
                    }
                ...
                state.output.println("Avg Depth: " + ((double)totDepthPerGen)/
                    (state.population.subpops.get(x).individuals.size() * numDepth.length), statisticslog);
                state.output.print("Depth/tree: [", statisticslog);
                for(int tr=0;tr<numDepth.length;tr++)
                    {
                    if (tr>0) state.output.print("|", statisticslog);
                    state.output.print(""+((double)numDepth[tr])/state.population.subpops.get(x).individuals.size(), statisticslog);
                    }
                state.output.println("]", statisticslog);
            ...
            if (!(state.population.subpops.get(x).species.f_prototype instanceof KozaFitness))
                state.output.fatal("Subpopulation " + x +
            ...
            for(int y=0;y<state.population.subpops.get(x).individuals.size();y++)
                ...
                // mean for population
                meanStandardized += ((KozaFitness)(state.population.subpops.get(x).individuals.get(y).fitness)).standardizedFitness();
                meanAdjusted += ((KozaFitness)(state.population.subpops.get(x).individuals.get(y).fitness)).adjustedFitness();
                hits += ((KozaFitness)(state.population.subpops.get(x).individuals.get(y).fitness)).hits;
                }

            // compute fitness stats
            meanStandardized /= state.population.subpops.get(x).individuals.size();
            meanAdjusted /= state.population.subpops.get(x).individuals.size();
            state.output.print("Mean fitness raw: " + meanStandardized + " adjusted: " + meanAdjusted + " hits: " + ((double)hits)/state.population.subpops.get(x).individuals.size(), statisticslog);
                    
            state.output.println("", statisticslog);

            // compute inds stats
            numInds += state.population.subpops.get(x).individuals.size();
            }
