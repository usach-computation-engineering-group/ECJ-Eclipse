...
public class CudaMutationPipeline extends GPBreedingPipeline {
	...
	public int produce(final int min, final int max, final int start,
		...
		int n = sources[0].produce(min, max, subpopulation, new ArrayList<Individual>(Arrays.asList(inds)), state,
				thread, null);

		// should we bother?
		if (!state.random[thread].nextBoolean(likelihood))
			return produce(n, n, subpopulation, new ArrayList<Individual>(Arrays.asList(inds)), state, thread,
					null); // DON'T produce children from source -- we already
							// did
