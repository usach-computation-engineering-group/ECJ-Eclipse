...
public class CudaSubpopulation extends Subpopulation {
	...
	public void populate(EvolutionState state, int thread) {
		...
		int len = individuals.size(); // original length of individual array
		...
		if (loadInds) {
			...
			if (len > individuals.size()) // the population was shrunk, there's
											// more space yet
			{
				...
				if (extraBehavior == TRUNCATE) {
					...
					state.output.message("Only " + individuals.size() + " individuals were read in.  Subpopulation will stay size " + len + ", and the rest will be filled with copies of the read-in individuals.");

					Individual[] oldInds = individuals.toArray(new Individual[individuals.size()]);
					individuals = new ArrayList<Individual>(Arrays.asList(new Individual[len]));
					System.arraycopy(oldInds, 0, individuals, 0, oldInds.length);
					start = oldInds.length;

					int count = 0;
					for (int i = start; i < individuals.size(); i++) {
						individuals.set(i, (Individual) (individuals.get(count).clone()));
						if (++count >= start)
							count = 0;
					}
					return;
				...
				{
					state.output.message("Only " + individuals.size() + " individuals were read in.  Subpopulation will stay size " + len + ", and the rest will be filled using randomly generated individuals.");
		...
		if (numDuplicateRetries >= 1)
			h = new HashMap((individuals.size() - start) / 2); // seems
																// reasonable

		for (int x = start; x < individuals.size(); x++) {
			for (int tries = 0; tries <= /* Yes, I see that */numDuplicateRetries; tries++) {
				individuals.set(x, species.newIndividual(state, thread));

				if (numDuplicateRetries >= 1) {
					// check for duplicates
					Object o = h.get(individuals.get(x));
					if (o == null) // found nothing, we're safe
					// hash it and go
					{
						// Add this individual to the current thread's
						// unevaluated individuals
						myUnevals.add((GPIndividual) individuals.get(x));
						h.put(individuals.get(x), individuals.get(x));
						break;
					}
				}
