...
public class CudaCrossoverPipeline extends CrossoverPipeline {
	...
	public int produce(final int min, 
	        ...
	        if (!state.random[thread].nextBoolean(likelihood))
	            ...
	            sources[0].produce(n,n,subpopulation, new ArrayList<Individual>(Arrays.asList(inds)), state,thread,null);
	            return n;
	            }
	        ...
	        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
	            ...
	            if (sources[0]==sources[1])  // grab from the same source
	                sources[0].produce(2,2,subpopulation,parents,state,thread,null);
	            else // grab from different sources
	                {
	                sources[0].produce(1,1,subpopulation,parents,state,thread,null);
	                sources[1].produce(1,1,subpopulation,parents,state,thread,null);
	                }
	            ...
	            CudaSubpopulation subPop = (CudaSubpopulation) state.population.subpops.get(subpopulation);
